...
pkl_file_path = './his_data.pkl'

with open(pkl_file_path, 'rb') as file:
    opt_params = pickle.load(file)
opt_para_list=[]
params_dict=opt_params[-1]['r=1.9']['e0']['params']
last_key = max(params_dict.keys())
opt_para_list.append(np.array(opt_params[-1]['r=1.9']['e0']['params'][last_key]))
params_dict=opt_params[-1]['r=1.9']['e1']['params']
last_key = max(params_dict.keys())
opt_para_list.append(np.array(opt_params[-1]['r=1.9']['e1']['params'][last_key]))

n_qubits_used = 6
print(f'Number of qubits used: {n_qubits_used}')

#generate Hartree-Fock state from Symmer
filename = '../../hamiltonian_data/BeH+_STO-3G_SINGLET_JW.json'
with open(filename, 'r') as infile:
    data_dict = json.load(infile)
hf_state   = QuantumState(np.asarray(data_dict['data']['hf_array']))

basis = 'sto-3g'; multiplicity = 1; charge = +1
number_of_points=51
his_data=[]
...
for i in range(number_of_points):
    ...
    vqd = VQD(observable=H_cs,ref_state=hf_cs,excitation_ops=UCC_cs,excited_order=1, beta_list=[10], method='BFGS',ref_state_same=True,para_list=opt_para_list)

    vqd.verbose = 1
    vqd_result, interim_data, opt_para_list= vqd.run()
